...
void MainWindow::CreateWidgets() {
  ...
  custom_widget_ = new CustomWidget(this, &options_);
  custom_widget_->SetDatabasePath(*options_.database_path);
  custom_widget_->SetImagePath(*options_.image_path);
  
  custom_feature_extraction_widget_ = new CustomFeatureExtractionWidget(this, &options_);
  custom_feature_matching_widget_ = new CustomFeatureMatchingWidget(this, &options_);
  ...
  addDockWidget(Qt::LeftDockWidgetArea, dock_log_widget_);

  custom_widget2_ = new CustomWidget2(this);
  custom_widget3_ = new CustomWidget3(this);
//  dock_info_widget_ = new QDockWidget("Info", this);
//  dock_info_widget_->setWidget(info_widget_);
//  addDockWidget(Qt::LeftDockWidgetArea, dock_info_widget_);
}
...
void MainWindow::CreateActions() {
  ...
  action_automatic_reconstruction_ =
      new QAction(QIcon(":/media/3d_reconstruction_icon.png"),
  ...
  custom_action_1_ = new QAction(QIcon(":/media/3d_reconstruction_icon.png"),tr("Action One"),this);
  connect(custom_action_1_, &QAction::triggered, this, &MainWindow::Action_1);

  custom_action_2_ = new QAction(QIcon(":/media/generate_mesh_icon.png"),tr("Action Two"),this);
  connect(custom_action_2_, &QAction::triggered, this, &MainWindow::Action_2);

  custom_action_3_ = new QAction(QIcon(":/media/style_transfer_icon.png"),tr("Action Three"),this);
  connect(custom_action_3_, &QAction::triggered, this, &MainWindow::Action_3);
}
...
//  extras_toolbar_->setIconSize(QSize(16, 16));
  ...
  addToolBarBreak();
  custom_sdu_toolbar_ = addToolBar(tr("sdu"));
  //custom_sdu_toolbar_->addAction(custom_action_1_);
  custom_sdu_toolbar_->addAction(action_automatic_reconstruction_);
  custom_sdu_toolbar_->setIconSize(QSize(320,22));

  custom_sdu_toolbar_->addAction(custom_action_2_);
  custom_sdu_toolbar_->setIconSize(QSize(320,22));
  custom_sdu_toolbar_->addAction(custom_action_3_);
  custom_sdu_toolbar_->setIconSize(QSize(320,22));
}
...
void MainWindow::UpdateWindowTitle() {
  if (*options_.project_path == "") {
    setWindowTitle(QString::fromStdString("3D Reconstruction and Style Transfer"));
  } else {
...
void MainWindow::Action_3()
{
    custom_widget3_->show();
    custom_widget3_->raise();
}

void MainWindow::Action_2()
{

//        custom_widget2_->Reset();
    custom_widget2_->show();
    custom_widget2_->raise();
//        custom_widget2_->hide();

}

void MainWindow::Action_1()
{
    if(ReconstructionOverwrite())
    {
        //custom_widget_->Reset();
        //custom_widget_->show();
       // custom_widget_->hide();
        if(custom_widget_->NewProjectOneClick()){
            custom_feature_extraction_widget_->show();
            custom_feature_extraction_widget_->hide();
            custom_feature_extraction_widget_->Extract();

            custom_feature_matching_widget_->show();
            custom_feature_matching_widget_->CustomOneClick();
            custom_feature_matching_widget_->hide();
        }
        else{
